...
class ASN1ProtoToDER {
 ...
 private:
  ...
  // number of bytes used to encode the identifier.
  ...
  // Writes |raw_len| to |encoder_| at |len_pos| and returns the number of
  // bytes used to encode |raw_len|.
  ...
  // |encoder_|, returning number of bytes used to encode indefinite-length
  // indicator and EOC.
  ...
  // of bytes used to encode |actual_len|.
  ...
  // number of bytes used to encode |val|.
  size_t EncodeValue(const Value& val);

  // Converts |value| to a variable-length, big-endian representation and inserts
