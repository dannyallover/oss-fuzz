...
uint8_t ASN1PDUProtoToDER::GetNumBytes(const size_t num) {
...
size_t ASN1PDUProtoToDER::EncodeIndefiniteLength(const size_t len_pos) {
  ...
  // The PDU's value is from |len_pos| to the end of |encoder_|, so just add
  // an EOC marker to the end.
  AppendBytes(0x00, encoder_.size());
  AppendBytes(0x00, encoder_.size());
  return 3;
}

size_t ASN1PDUProtoToDER::EncodeCorrectLength(const size_t actual_len,
                                              const size_t len_pos) {
...
uint64_t ASN1PDUProtoToDER::EncodeHighTagForm(const uint8_t id_class,
...
size_t ASN1PDUProtoToDER::EncodeIdentifier(const Identifier& id) {
...
void ASN1PDUProtoToDER::ParseToBits() {
