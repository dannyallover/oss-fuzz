...
class ASN1PDUProtoToDER {
 ...
 private:
  ...
  // in bytes of the encoded length. |len_pos| contains the offset in
  // |encoder_| where the length should be encoded. |len| can be used to
  // affect the encoding, in order to produce invalid lengths. To correctly call
  // this, the tag must already be encoded immediately prior to |len_pos|, and
  // the remainder of |encoder_| represents the encoded value.
  size_t EncodeLength(const Length& len, size_t actual_len, size_t len_pos);
  // Writes |raw_length| to |encoder_| at |len_pos| and returns the number
  // of bytes written to |encoder_|.
  ...
  // |encoder_|, returning number of bytes needed to encode
  // indefinite-length indicator and EOC.
  size_t EncodeIndefiniteLength(const size_t len_pos);
  // Encodes the length in |actual_len| using the definite-form length (X.690
  // (2015), 8.1.3-8.1.5 & 10.1) into |encoder_| at |len_pos|, returning
  // number of bytes needed to encode |actual_len|.
